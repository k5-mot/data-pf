    ...
    return {
        "range_checks": {
            ...
            "volume_ratio": {
                "min": 0,
                "max": 50,
                "description": "Volume ratio should be reasonable",
            },
            "volatility_20d": {
                "min": 0,
                "max": 1.0,
                "description": "Volatility should be between 0 and 100%",
            },
        },
        "relationship_checks": {
            "bollinger_bands": {
...
def validate_range_checks(df: DataFrame, rules: dict[str, Any]) -> dict[str, Any]:
    ...
    for field, rule in rules.items():
        ...
        # 範囲外の値をカウント
        out_of_range_condition = (min_val is not None and col(field) < min_val) | (
            max_val is not None and col(field) > max_val
        )

        out_of_range_count = df.filter(
            out_of_range_condition & col(field).isNotNull()
        ).count()
        ...
        logger.info(
            f"Range check {field}: {results[field]['status']} (error rate: {error_rate:.3f})"
        )

    return results


def validate_relationship_checks(
    df: DataFrame, rules: dict[str, Any]
) -> dict[str, Any]:
    """関係性チェック
...
def validate_completeness_checks(
    df: DataFrame, rules: dict[str, Any]
) -> dict[str, Any]:
    """完整性チェック
    ...
    for field in required_fields:
        ...
        completeness_rate = (
            (total_records - null_count) / total_records if total_records > 0 else 0
        )
        ...
        logger.info(
            f"Completeness {field}: {results[field]['status']} (rate: {completeness_rate:.3f})"
        )
...
def validate_consistency_checks(df: DataFrame, rules: dict[str, Any]) -> dict[str, Any]:
    ...
    if "temporal_continuity" in rules:
        ...
        gap_violations = (
            df.sql_ctx.sql(gap_check_sql).collect() if hasattr(df, "sql_ctx") else []
        )
    ...
    if "value_continuity" in rules:
        ...
        rsi_change_df = df.withColumn(
            "prev_rsi", col("rsi_14d").lag(1).over(window_spec)
        ).withColumn(
            "rsi_change_pct",
...
def calculate_overall_quality_score(
    ...
    for category, results in validation_results.items():
        ...
        passed_tests = sum(
            1 for result in results.values() if result.get("status") == "PASS"
        )
...
def generate_validation_report(
    df: DataFrame, validation_results: dict, quality_score: dict
) -> str:
    """検証レポート生成
    ...
    for category, results in validation_results.items():
        ...
        for test_name, result in results.items():
            ...
            if "completeness_rate" in result:
                report_lines.append(
                    f"    Completeness: {result['completeness_rate']:.3f}"
                )
    ...
    if quality_score["total_score"] < 0.70:
        report_lines.append(
            "  ⚠️  URGENT: Data quality issues detected. Review and fix data pipeline."
        )
    elif quality_score["total_score"] < 0.85:
        report_lines.append(
            "  ⚠️  WARNING: Some data quality issues found. Monitor closely."
        )
    else:
...
def save_validation_results(
    spark: SparkSession,
    results: dict,
    table_name: str = "quality.technical_features_validation",
):
    """検証結果をテーブルに保存
    ...
    try:
        ...
        logger.error(f"Failed to save validation results: {e!s}")
...
def main():
    ...
    try:
        ...
        logger.info(
            f"Validation completed. Quality score: {quality_score['total_score']:.3f}"
        )
        ...
        if (
            ...
            < validation_rules["thresholds"]["error_rate_threshold"]
        ):
        ...
        logger.error(f"Fatal error in validation process: {e!s}")
        return 1
